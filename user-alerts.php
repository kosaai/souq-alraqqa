      ...
      <div class="inside">
        <?php if(osc_count_alerts() > 0) { ?>
          ...
          <?php while(osc_has_alerts()) { ?>
            ...
            <div class="userItem" >
              <div class="hed">
                <span><?php echo osc_count_items(); ?> <?php echo __('نتائج)', 'starter'); ?></span>
                <a class="tr1 has-tooltip" onclick="javascript:return confirm('<?php echo osc_esc_js(__('لا يمكن التراجع عن هذا الإجراء. هل أنت متأكد أنك تريد الاستمرار؟', 'starter')); ?>');" href="<?php echo osc_user_unsubscribe_alert_url(); ?>" title="<?php echo osc_esc_html(__('إلغاء الاشتراك', 'starter')); ?>"><i class="fa fa-unlink"></i></a>
              </div>

              <div class="hed-param">
                <div class="elem <?php if($alert_details['sPattern'] == '') { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('نمط', 'starter'); ?>:</div>
                  <div class="right"><?php if($alert_details['sPattern'] == '') { _e('لا أحد', 'starter'); } else { echo $alert_details['sPattern']; } ?></div>
                </div>

                <div class="elem <?php if($alert_details['price_min'] == 0) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('دقيقة. سعر', 'starter'); ?>:</div>
                  <div class="right"><?php if($alert_details['price_min'] == 0) { _e('لا أحد', 'starter'); } else { echo $alert_details['price_min'] . osc_get_preference('def_cur', 'starter_theme'); } ?></div>
                </div>

                <div class="elem <?php if($alert_details['price_max'] == 0) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('الأعلى. سعر', 'starter'); ?>:</div>
                  <div class="right"><?php if($alert_details['price_max'] == 0) { _e('لا أحد', 'starter'); } else { echo $alert_details['price_max'] . osc_get_preference('def_cur', 'starter_theme'); } ?></div>
                </div>

                <div class="elem <?php if($alert_details['countries'] == '' or empty($c_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('دولة', 'starter'); ?>:</div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem <?php if($alert_details['regions'] == '' or empty($r_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('منطقة', 'starter'); ?>:</div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem <?php if($alert_details['cities'] == '' or empty($t_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('مدينة', 'starter'); ?>:</div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem <?php if($alert_details['aCategories'] == '' or empty($cat_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('فئات', 'starter'); ?>:</div>
                  <div class="right">
                    <?php 
              ...
              <?php while(osc_has_items()) { ?>
                ...
                <div class="item-entry round3 tr1">
                  <?php if( osc_images_enabled_at_items() ) { ?>
                    ...
                    <?php } else { ?>
                      <a class="photo tr1" href="<?php echo osc_item_url(); ?>"><img class="tr1" src="<?php echo osc_current_web_theme_url('images/no-image.png'); ?>" title="<?php echo osc_esc_html(__('لا توجد صورة', 'starter')); ?>" alt="<?php echo osc_esc_html(__('لا توجد صورة', 'starter')); ?>" width="150" height="125"/></a>
                    <?php } ?>
                  <?php } ?>
                    ...
                    <div class="middle">
                      <?php if(osc_item_total_comments() > 0) { ?>
                        <?php if(osc_item_total_comments() == 1) { ?>
                          <div class="comment"><?php echo sprintf(__('%d مراجعة', 'starter'), osc_item_total_comments()); ?></div>
                        <?php } else { ?>
                          <div class="comment"><?php echo sprintf(__('%d التعليقات', 'starter'), osc_item_total_comments()); ?></div>
                        <?php } ?>
                      <?php } ?>
                      ...
                      <?php if($location <> '') { ?>
                        <div class="location has-tooltip" title="<?php echo osc_esc_html(__('موقع', 'starter')); ?>"><?php echo $location; ?></div>
                      <?php } ?>

                      <div class="published has-tooltip" title="<?php echo osc_esc_html(__('تاريخ النشر', 'starter')); ?>"><?php echo date('Y/m/d', strtotime(osc_item_pub_date())); ?></div>
        ...
        <?php } else { ?>
          <div class="ua-items-empty ua-alerts-total">
            <span><?php _e('ليس لديك أي اشتراكات حتى الآن', 'starter'); ?></span>
            <span class="details"><?php echo sprintf(__('أنشئت واحدة، وانتقل إلى %s و على زر "الاشتراك".', 'starter'), '<a href="' . osc_search_url(array('page' => 'search')) . '">' . __('يبحث', 'starter') . '</a>'); ?></span>
          </div>
        <?php  } ?>
      </div>
    </div>
