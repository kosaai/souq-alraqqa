...
function starter_backoffice_menu($title = NULL) {
  ...
  if( $title == '') { $title = __('إعدادات', 'starter'); }
  ...
  $text .= '<li><a href="' . osc_base_url() . 'oc-admin/index.php?page=appearance&action=render&file=oc-content/themes/starter/admin/settings.php"><i class="fa fa-wrench"></i><span>' . __('إعدادات', 'starter') . '</span></a></li>';
  $text .= '<li><a href="' . osc_base_url() . 'oc-admin/index.php?page=appearance&action=render&file=oc-content/themes/starter/admin/header.php"><i class="fa fa-desktop"></i><span>' . __('رأس', 'starter') . '</span></a></li>';
  $text .= '</ul>';
  $text .= '</div>';
  $text .= '</div>';
...
function starter_footer() {
  ...
  $text .= '<a target="_blank" href="' . $themeInfo['support_uri'] . '"><i class="fa fa-bug"></i> ' . __('الإبلاغ عن الخطأ', 'starter') . '</a>';
  $text .= '<a target="_blank" href="https://forums.mb-themes.com/"><i class="fa fa-comments"></i> ' . __('منتديات الدعم', 'starter') . '</a>';
  $text .= '<a target="_blank" class="mb-last" href="mailto:info@mb-themes.com"><i class="fa fa-envelope"></i> ' . __('اتصل بنا', 'starter') . '</a>';
...
function starter_has_subcategories_special($categories, $deep = 0) {
  ...
  foreach($categories as $c) {
    ...
    echo '<div class="mb-col-1_1_2"><a class="add_img" id="small' . $c['pk_i_id'] . '" href="#">' . __('أضف صورة صغيرة', 'starter') . '</a></div>';
    ...
    echo '<div class="mb-col-1_1_2"><a class="add_img" id="large' . $c['pk_i_id'] . '" href="#">' . __('إضافة صورة كبيرة', 'starter') . '</a></div>';
    echo '<div class="mb-col-1_1_2 mb-align-left fa-icon"><a class="add_fa" id="fa-icon' . $c['pk_i_id'] . '" href="#" title="To remove icon click on link and leave input empty.">' . __('إضافة / إزالة أيقونة', 'starter') . '</a>';
    ...
    if(isset($_POST['color' .$c['pk_i_id']]) and $_POST['color' .$c['pk_i_id']] == '') { 
      echo  __('إضافة / إزالة اللون', 'starter');
      ...
      echo __('لون', 'starter') . ': ' . $color . '<span class="show-color" style="background:' . $color . '"></span>';
    }
