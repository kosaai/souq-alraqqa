...
<div class="content list">
  <div id="main" class="search">
    ...
    <div id="search-items" data-loading="<?php _e('جارٍ تحميل القوائم...', 'starter'); ?>">
      ...
      <?php if(osc_count_items() == 0) { ?>
        <div class="list-empty round3" >
          ...
          <div>
            <span><?php _e('عفوًا، لا توجد قائمة تطابق معايير البحث...', 'starter'); ?></span>
          </div>
        </div>
      <?php } else { ?>
  ...
  <div id="sidebar" class="noselect">
    <div id="sidebar-search">
      <div class="side-cat">
        ...
        <h3>
          <span><?php _e('فئات', 'starter'); ?></span>

          <?php if($search_cat_id > 0) { ?>
            <?php $search_params['sCategory'] = @$parent['fk_i_parent_id']; ?>
            <a href="<?php echo osc_search_url($search_params); ?>" class="gotop"><?php _e('الوالدين', 'starter'); ?></a>
          <?php } ?>
        </h3>
      ...
      <form action="<?php echo osc_base_url(true); ?>" method="get" onsubmit="" class="search-side-form nocsrf">
        ...
        <div class="wrap">
          ...
          <h3 class="head">
            <span><?php _e('يبحث', 'starter'); ?></span>
            <a href="#" class="show-advanced"><?php _e('متقدم', 'starter'); ?></a>
          </h3>
          ...
          <div class="search-wrap">
            ...
            <fieldset class="box location">
              <div class="row">
                <h4><?php _e('الكلمة الرئيسية', 'starter') ; ?></h4>                            
                <div class="input-box">
                  <i class="fa fa-pencil"></i>
                  <input type="text" name="sPattern" id="query" value="<?php echo osc_esc_html(osc_search_pattern()); ?>" placeholder="<?php echo osc_esc_html(__('يبحث...', 'starter')); ?>" />
                </div>
              </div>

              <div class="row">
                <h4><?php _e('موقع', 'starter') ; ?></h4>                            

                <div class="box">
                  <div id="location-picker">
                    <input type="text" name="term" id="term" class="term" placeholder="<?php _e('البلد أو المنطقة أو المدينة', 'starter'); ?>" value="<?php echo starter_get_term(Params::getParam('term'), Params::getParam('sCountry'), Params::getParam('sRegion'), Params::getParam('sCity')); ?>" autocomplete="off"/>
                ...
                <fieldset>
                  <div class="row checkboxes">
                    <div class="input-box-check">
                      ...
                      <label for="withPicture" class="with-pic-label"><?php _e('العناصر التي تحتوي على صورة فقط', 'starter'); ?></label>
                    </div>
                  </div>
                </fieldset>
              <?php } ?>
            ...
            <div class="button-wrap">
              <button type="submit" class="btn btn-primary" id="search-button"><?php _e('يبحث', 'starter') ; ?></button>
              <a href="<?php echo osc_search_url(array('page' => 'search'));?>" class="clear-search clear-cookie btn btn-secondary"><?php _e('إعادة ضبط', 'starter'); ?></a>
            </div>
          </div>
        </div>
