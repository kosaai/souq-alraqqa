  ...
  <div class="inside">
    ...
    <div class="right-block not767">
      <a class="publish round3 tr1" href="<?php echo osc_item_post_url(); ?>">
        <span class="non-resp"><?php _e('إضافة قائمة', 'starter'); ?></span>
        <span class="resp"><?php _e('يضيف', 'starter'); ?></span>
      </a>

      <div class="user-header-block">
        <a class="picture tr1<?php if(osc_is_web_user_logged_in()) { ?> has-menu<?php } ?>" href="<?php echo osc_user_dashboard_url(); ?>"><img src="<?php echo starter_profile_picture(osc_logged_user_id(), 'small', true); ?>" alt="<?php echo osc_esc_html(__('صورة المستخدم', 'starter')); ?>"/></a>
      ...
      <div class="search-top">
        <form action="<?php echo osc_base_url(true); ?>" method="get" class="nocsrf spin" >
          ...
          <input type="text" name="sPattern" id="query" value="<?php echo osc_esc_html(osc_search_pattern()); ?>" placeholder="<?php _e('ابحث...', 'starter'); ?>" autocomplete="off" />
    ...
    <div class="left-block c<?php echo $c; ?>">
      ...
      <?php if(osc_is_web_user_logged_in()) { ?>
        ...
        <a class="le-btn listings not767" href="<?php echo osc_user_list_items_url(); ?>">
          ...
          <span><?php _e('القوائم', 'starter'); ?></span> 
      ...
      <?php if(function_exists('im_messages') && (osc_is_web_user_logged_in() || osc_get_preference('link_reg_only','plugin-instant_messenger') <> 1)) { ?>
        <a class="le-btn messages" href="<?php echo osc_route_url( 'im-threads'); ?>">
          ...
          <span><?php _e('رسائل', 'starter'); ?></span> 
    ...
    <h1>
      <?php 
        if(osc_is_home_page()) {
          ...
          echo ' - ' . osc_search_total_items() . ' ' . (osc_search_total_items() == 1 ? __('نتيجة', 'starter') : __('نتائج', 'starter'));

        } else if ($loc == 'register') {
          _e('تسجيل الدخول', 'starter');

        } else if ($loc == 'login' && $sec == 'recover') {
          _e('استعادة كلمة المرور', 'starter');

        } else if ($loc == 'item' && $sec == 'item_add') {
          _e('نشر قائمة جديدة', 'starter');

        } else if ($loc == 'item' && $sec == 'item_edit') {
          _e('قم بتحرير قائمتك', 'starter');

        } else if ($loc == 'user' && $sec == 'pub_profile') {
          echo sprintf(__('الملف الشخصي لـ %s', 'starter'), osc_user_name());

        } else if ($loc == 'user' && $sec == 'items') {
          _e('عناصر المستخدم', 'starter');

        } else if ($loc == 'user' && $sec == 'alerts') {
          _e('تنبيهات المستخدم', 'starter');

        } else if ($loc == 'user' && $sec == 'profile') {
          _e('ملف تعريف المستخدم', 'starter');
