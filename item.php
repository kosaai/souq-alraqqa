...
<?php 
  ...
  if(osc_item_price() == '') {
    $og_price = __('تحقق مع البائع', 'starter');
  } else if(osc_item_price() == 0) {
    $og_price = __('حر', 'starter');
  } else {
...
<body id="body-item" class="page-body<?php if($content_only == 1) { ?> content_only<?php } ?><?php if ($ios) { ?> ios<?php } ?>">
  <?php if($content_only == 0) { ?>
    ...
    <?php if( osc_item_is_expired () ) { ?>
      <div class="exp-box">
        <div class="exp-mes round3"><?php _e('انتهت صلاحية هذه القائمة.', 'starter'); ?></div>
      </div>
    <?php } ?>
  <?php } ?>
    ...
    <div id="main">
      ...
      <div id="left">
                ...
                <div id="pictures" class="item-pictures">
                  <ul class="item-bxslider">
                    ...
                    <?php for( $i = 0; osc_has_item_resources(); $i++ ) { ?>
                      <li>
                        <?php if($content_only == 0) { ?>
                          <a rel="image_group" data-fancybox="gallery" href="<?php echo osc_resource_url(); ?>" title="<?php echo osc_esc_html(osc_item_title()); ?> - <?php _e('صورة', 'starter'); ?> <?php echo $i+1;?>/<?php echo osc_count_item_resources();?>">
                  ...
                  <div id="photo-count" class="round2">
                    ...
                    <div class="bottom">
                      <?php if(osc_count_item_resources() == 1) { ?>
                        <span class="p-total"><?php echo osc_count_item_resources(); ?></span> <?php _e('صورة', 'starter'); ?>
                      <?php } else { ?>
                        <span class="p-from">1</span> <span class="p-del">-</span> <span class="p-to"><?php echo $at_once; ?></span> <?php _e('ل', 'starter'); ?> <span class="p-total"><?php echo osc_count_item_resources(); ?></span>
                      <?php } ?>
                    </div>
                  </div>
                  ...
                  <?php if(osc_count_item_resources() > 1 && osc_get_preference('item_pager', 'starter_theme') == 1) { ?>
                    <div id="item-bx-pager">
                      ...
                      <?php for( $i = 1; osc_has_item_resources(); $i++ ) { ?>
                        ...
                        <img src="<?php echo osc_resource_thumbnail_url(); ?>" alt="<?php echo osc_esc_html(osc_item_title()); ?> - <?php _e('صورة', 'starter'); ?> <?php echo $i;?>/<?php echo osc_count_item_resources();?>"/>
        ...
        <?php if($content_only == 0) { ?>
          <div class="item-desc item-block" data-block="detail">
            ...
            <div id="item-basics">
              <div class="elem"><?php _e('بطاقة تعريف', 'starter'); ?> #<?php echo osc_item_id(); ?></div>

              <div class="elem">
                <?php if (osc_item_mod_date() == '') { ?>
                  <span title="<?php echo osc_format_date(osc_item_pub_date()); ?>"><?php _e('تم النشر', 'starter'); ?> <?php echo starter_smart_date(osc_item_pub_date()); ?></span>
                <?php } else { ?>
                  <span title="<?php echo osc_format_date(osc_item_pub_date()); ?>"><?php _e('معدل', 'starter'); ?> <?php echo starter_smart_date(osc_item_mod_date()); ?></span>
                <?php } ?>
              </div>   
            </div>
            ...
            <div class="mobile-labels is767">
              <?php
                if($item_extra['i_sold'] == 1) {
                  echo '<div class="elem sold">' . __('مُباع', 'starter') . '</div>';
                } else if($item_extra['i_sold'] == 2) { 
                  echo '<div class="elem reserved">' . __('محجوز', 'starter') . '</div>';
                }

                if(osc_item_is_premium()) {
                  echo '<div class="elem premium">' . __('غالي', 'starter') . '</div>';
                }

                if(!in_array(osc_item_category_id(), starter_extra_fields_hide())) {
                  if(starter_condition_name($item_extra['i_condition'])) {
                    echo '<div class="elem condition">' . __('حالة', 'starter') . ': <span>' . starter_condition_name($item_extra['i_condition']) . '</span></div>';
                  }

                  if(starter_condition_name($item_extra['i_transaction'])) {
                    echo '<div class="elem transaction">' . __('عملية', 'starter') . ': <span>' . starter_transaction_name($item_extra['i_transaction']) . '</span></div>';
                  }
                }
              ?>
            </div>
            ...
            <h2 class="attribute"><?php _e('صفات', 'starter'); ?></h2>
      ...
      <?php if($content_only == 0) { ?>
        <div id="more-info" class="contact-seller item-block" data-block="contact">
          ...
          <h2><span><?php _e('اتصل بالبائع', 'starter'); ?></span></h2>
            ...
            <form action="<?php echo osc_base_url(true) ; ?>" method="post" name="contact_form" id="contact_form" <?php if(osc_item_attachment()) { echo 'enctype="multipart/form-data"'; };?>>
              ...
              <fieldset>
                <div class="message-block">
                  <?php if( osc_item_is_expired () ) { ?>
                    <div class="empty">
                      <?php _e('انتهت هذه القائمة، لا يمكنك الاتصال بالبائع.', 'starter') ; ?>
                    </div>
                  <?php } else if( (osc_logged_user_id() == osc_item_user_id()) && osc_logged_user_id() != 0 ) { ?>
                    <div class="empty">
                      <?php _e('إنها قائمتك الخاصة، لا يمكنك الاتصال بنفسك.', 'starter') ; ?>
                    </div>
                  <?php } else if( osc_reg_user_can_contact() && !osc_is_web_user_logged_in() ) { ?>
                    <div class="empty">
                      <?php _e('يجب عليك تسجيل الدخول أو تسجيل حساب جديد حتى تتمكن من التواصل مع المعلن.', 'starter') ; ?>
                    </div>
                  <?php } else { ?> 
                    <div class="row first">
                    ...
                    <div class="row third">
                      <label for="phoneNumber"><span><?php _e('هاتف', 'starter'); ?></span></label>
                      <?php ContactForm::your_phone_number(); ?>
                    </div>

                    <div class="row full">
                      <label for="message"><span><?php _e('رسالة', 'starter'); ?></span></label>
                      <?php ContactForm::your_message(); ?>
                    </div>
                    
                    <?php if(osc_item_attachment()) { ?>
                      <div class="att-wrap">
                        <label for="attachment"><span><?php _e('مرفق', 'starter'); ?></span></label>

                        <div class="attachment">
                          <div class="att-box">
                            <label class="status">
                              <span class="wrap"><i class="fa fa-paperclip"></i> <span data-original="<?php echo osc_esc_html(__('تحميل الملف', 'starter')); ?>"><?php _e('تحميل الملف', 'starter'); ?></span></span>
                    ...
                    <button type="<?php echo (osc_get_preference('forms_ajax', 'starter_theme') == 1 ? 'button' : 'submit'); ?>" class="send" id="item-message"><?php _e('أرسل رسالة', 'starter') ; ?></button>
                  <?php } ?>
                </div>
                ...
                <div class="message-status message-sent">
                  ...
                  <div class="title"></div>
                  <div class="link"><a href="#" class="next-message"><?php _e('إرسال الرسالة التالية', 'starter'); ?></a></div>
                </div>
        ...
        <div id="more-info" class="comments item-block" data-block="comments">
          <?php if( osc_comments_enabled()) { ?>
            <div class="item-comments">
              <h2 class="sc-click">
                <span><?php _e('تعليقات', 'starter'); ?></span>
              </h2>
              ...
              <div id="comments" class="sc-block">
                <div class="comments_list">
                  ...
                  <?php while ( osc_has_item_comments() ) { ?>
                    <div class="comment-wrap <?php echo $class; ?>">
                      <div class="ins">
                        ...
                        <div class="comment">
                          <h4><span class="bold"><?php if(osc_comment_title() == '') { _e('مراجعة', 'starter'); } else { echo osc_comment_title(); } ?></span> <?php _e('بواسطة', 'starter') ; ?> <?php if(osc_comment_author_name() == '') { _e('مجهول', 'starter'); } else { echo osc_comment_author_name(); } ?>:</h4>
                          <div class="body"><?php echo osc_comment_body() ; ?></div>

                          <?php if ( osc_comment_user_id() && (osc_comment_user_id() == osc_logged_user_id()) ) { ?>
                            <a rel="nofollow" class="remove" href="<?php echo osc_delete_comment_url(); ?>" title="<?php echo osc_esc_html(__('احذف تعليقك', 'starter')); ?>">
                              <span class="not767"><?php _e('يمسح', 'starter'); ?></span>
                  ...
                  <?php if($i == 1) { ?>
                    <div class="no-comments"><?php _e('لم يتم التعليق على هذه القائمة بعد', 'starter'); ?></div>
                  <?php } ?>
              ...
              <?php if( osc_reg_user_post_comments () && osc_is_web_user_logged_in() || !osc_reg_user_post_comments() ) { ?>
                <a class="add-com btn btn-primary" href="<?php echo osc_item_send_friend_url(); ?>"><?php _e('أضف تعليقا', 'starter'); ?></a>
              <?php } ?>
            </div>
          <?php } ?>
        </div>
      ...
      <div id="side-right">
            ...
            <div class="inside">
              ...
              <div class="side-photo">
                ...
                <div class="name">
                  <?php
                    $c_name = '';
                    if(osc_item_contact_name() <> '' and osc_item_contact_name() <> __('مجهول', 'starter')) {
                    ...
                    if($c_name == '') {
                      $c_name = __('مجهول', 'starter');
                    }
                  ?>

                  <?php if(osc_item_user_id() <> 0 and osc_item_user_id() <> '') { ?>
                    <a href="<?php echo osc_user_public_profile_url(osc_item_user_id()); ?>" title="<?php echo osc_esc_html(__('التحقق من الملف الشخصي لهذا المستخدم', 'starter')); ?>">
                      <?php echo $c_name; ?>
                    </a>
                  <?php } else { ?>
                    <?php echo $c_name; ?>
                ...
                <div class="elem regdate">
                  <?php if(osc_item_user_id() <> 0) { ?>
                    ...
                    <?php if(isset($get_user['dt_reg_date']) AND $get_user['dt_reg_date'] <> '') { ?>
                      <?php echo __('مسجل على', 'starter') . ' ' . osc_format_date( $get_user['dt_reg_date'] ); ?>
                    <?php } else { ?>
                      <?php echo __('تاريخ التسجيل غير معروف', 'starter'); ?>
                    <?php } ?>
                  <?php } else { ?>
                    <?php echo __('مستخدم غير مسجل', 'starter'); ?>
                  <?php } ?>
                </div>
              </div>
              ...
              <?php if(osc_is_web_user_logged_in() && osc_item_user_id() == osc_logged_user_id()) { ?>
                <div id="s-tools">
                  <a href="<?php echo osc_item_edit_url(); ?>" class="tr1 round2"><i class="fa fa-edit tr1"></i><span><?php _e('يحرر', 'starter'); ?></span></a>
                  <a href="<?php echo osc_item_delete_url(); ?>" class="tr1 round2" onclick="return confirm('<?php _e('هل أنت متأكد أنك تريد حذف هذه القائمة؟ لا يمكن التراجع عن هذا الإجراء.', 'starter'); ?>?')"><i class="fa fa-trash-o tr1"></i><span><?php _e('يزيل', 'starter'); ?></span></a>
                  ...
                  <?php if(!in_array(osc_item_category_id(), starter_extra_fields_hide())) { ?>
                    <a target="_blank" class="tr1 round2 sold" href="<?php echo osc_user_list_items_url() . $sold_url; ?>"><i class="fa fa-gavel"></i> <span><?php echo ($item_extra['i_sold'] == 1 ? __('لا تباع', 'starter') : __('مُباع', 'starter')); ?></span></a>
                    <a target="_blank" class="tr1 round2 reserved" href="<?php echo osc_user_list_items_url() . $reserved_url; ?>"><i class="fa fa-flag-o"></i> <span><?php echo ($item_extra['i_sold'] == 2 ? __('غير تحفظ', 'starter') : __('محجوز', 'starter')); ?></span></a>
                  <?php } ?>
                </div>
              <?php } ?>
            ...
            <div class="item-actions">
              <?php if($mobile_found) { ?>
                <div class="row phone">
                  ...
                  <?php if($mobile_login_required) { ?>
                    <a href="<?php echo osc_user_login_url(); ?>" class="phone-block login-required has-tooltip" data-item-id="<?php echo osc_item_id(); ?>" data-item-user-id="<?php echo osc_item_user_id(); ?>" title="<?php echo osc_esc_js(__('قم بالدخول ليظهر رقم الهاتف', 'starter')); ?>">
                      <span>
                        <?php echo __('قم بالدخول ليظهر الرقم', 'starter'); ?>
                      </span>
                    </a>
                  <?php } else { ?>
                    <a href="#" class="phone-block has-tooltip" data-item-id="<?php echo osc_item_id(); ?>" data-item-user-id="<?php echo osc_item_user_id(); ?>" title="<?php echo osc_esc_js(__('انقر لإظهار رقم الهاتف', 'starter')); ?>">
                      <span>
                        <?php 
                          if(strlen($mobile) >= 4 && $mobile <> __('لا يوجد رقم هاتف', 'starter')) {
              ...
              <?php if(osc_item_show_email()) { ?>
                <div class="row">
                  ...
                  <a href="#" class="mail-block has-tooltip" rel="<?php echo substr($mail, 3); ?>" title="<?php echo osc_esc_js(__('انقر لإظهار عنوان البريد الإلكتروني', 'starter')); ?>">
                    <span>
              ...
              <?php if(function_exists('im_manage_contact_seller')) { ?>
                <div class="row">
                  ...
                  <a class="send-message" href="<?php echo osc_route_url('im-create-thread', array('item-id' => osc_item_id())); ?>"><?php _e('أرسل رسالة', 'starter'); ?></a>
                </div>
              <?php } ?>
              ...
              <?php if(osc_item_user_id() <> 0) { ?>
                <div class="row dash">
                  ...
                  <?php if(function_exists('seller_post')) { ?>
                    <?php seller_post(); ?>
                  <?php } else { ?>
                    <a href="<?php echo osc_user_public_profile_url(osc_item_user_id()); ?>"><?php _e('لوحة القيادة', 'starter'); ?></a>
                  <?php } ?>
                </div>
              <?php } ?>
              ...
              <?php if(osc_item_user_id() <> 0) { ?>
                <div class="row type">
                  ...
                  <?php if($user['b_company'] == 1) { ?>
                    <i class="fa fa-briefcase"></i> <?php _e('شركة', 'starter'); ?>
                  <?php } else { ?>
                    <i class="fa fa-user-o"></i> <?php _e('شخص خاص', 'starter'); ?>
                  <?php } ?>
                </div>
              <?php } ?>
              ...
              <div class="row friend">
                ...
                <a id="send-friend" href="<?php echo osc_item_send_friend_url(); ?>"><?php _e('أرسل إلى صديق', 'starter'); ?></a>
              </div>
              ...
              <?php if (function_exists('show_printpdf')) { ?>
                <div class="row pdf">
                  ...
                  <a id="print_pdf" target="_blank" href="<?php echo osc_base_url(); ?>oc-content/plugins/printpdf/download.php?item=<?php echo osc_item_id(); ?>" ><?php _e('تحميل قوات الدفاع الشعبي', 'starter'); ?></a>
                </div>
              <?php } ?>
            </div>
          </div>
          ...
          <div class="body sc-block">
            <div class="loc-text">
              <?php if(trim(osc_item_country() . osc_item_region() . osc_item_city()) == '') {?>
                <div class="box-empty">
                  <span><?php _e('لم يتم تحديد الموقع', 'starter'); ?></span>
                </div>
              <?php } ?>
            ...
            <?php if($item_loc <> '') { ?>
              <a class="btn btn-primary" target="_blank" href="https://www.google.com/maps?daddr=<?php echo urlencode($item_loc); ?>"><?php _e('احصل على الاتجاهات', 'starter'); ?></a>
            <?php } ?>
          </div>  
        </div>
        ...
        <div id="user-card" class="item-block" data-block="contact">
          ...
          <div class="side-photo">
            ...
            <div class="name"><?php echo (osc_item_contact_name() <> '' ? osc_item_contact_name() :  __('مجهول', 'starter')); ?></div>
          </div>
          ...
          <div class="body sc-block">
            ...
            <div class="elem">
              <strong><?php _e('دولة', 'starter'); ?></strong> 
              <span><?php echo (@$user['s_country'] <> '' ? $user['s_country'] : (osc_item_country() <> '' ? osc_item_country() : '-')); ?></span>
            </div>

            <div class="elem">
              <strong><?php _e('منطقة', 'starter'); ?></strong> 
              <span><?php echo (@$user['s_region'] <> '' ? $user['s_region'] : (osc_item_region() <> '' ? osc_item_region() : '-')); ?></span>
            </div>

            <div class="elem">
              <strong><?php _e('عنوان', 'starter'); ?></strong> 
              <span><?php echo (@$user['s_city'] <> '' ? $user['s_city'] : (osc_item_city() <> '' ? osc_item_city() : '-')); ?></span>
            </div>

            <div class="elem">
              <strong><?php _e('يكتب', 'starter'); ?></strong> 
              <span><?php echo (@$user['s_country'] == 1 ? __('شركة', 'starter') : __('خاص', 'starter')); ?></span>
            </div>

            <div class="elem">
              <strong><?php _e('هاتف', 'starter'); ?></strong> 
              ...
              <?php
                ...
                if(osc_get_preference('reg_user_can_see_phone', 'osclass') == 1 && !osc_is_web_user_logged_in() && strlen(trim($mobile)) >= 4) {
                  $user_phone = __('تسجيل الدخول لرؤية رقم الهاتف', 'starter');
                }
              ?>
              ...
              <a href="#" class="phone-block" data-item-id="<?php echo osc_item_id(); ?>" data-item-user-id="<?php echo osc_item_user_id(); ?>">
                <span>
                  <?php 
                    if(strlen($user_phone) >= 4 && $user_phone <> __('لا يوجد رقم هاتف', 'starter') && $user_phone <> __('تسجيل الدخول لرؤية رقم الهاتف', 'starter')) {
            ...
            <?php if(osc_item_show_email()) { ?>
              <div class="elem">
                <strong><?php _e('بريد إلكتروني', 'starter'); ?></strong> 
  ...
  <?php if($content_only == 0) { ?>
    <script type="text/javascript">
      $(document).ready(function(){
        ...
        <?php if($mobile <> __('لا يوجد رقم هاتف', 'starter') && !$mobile_login_required) { ?>  
          $('.phone-show, .phone-block').click(function(e){
            ...
            if($('.phone-block').attr('href') == '#') {
              ...
              $('.phone-show').text('<?php echo osc_esc_js(__('انقر للاتصال', 'starter')); ?>');
        ...
        <?php if(osc_item_show_email()) { ?>  
          $('.mail-show, .mail-block').click(function(){
            ...
            if($('.mail-block').attr('href') == '#') {
              ...
              $('.mail-show').text('<?php echo osc_esc_js(__('انقر للبريد', 'starter')); ?>');
